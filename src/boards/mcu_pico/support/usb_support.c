...
// TX & RX buffers
uint8_t tx_buffer[64], rx_buffer[64];

// TX & RX status
volatile bool tx_done = true, rx_done = false;
...
static const struct usb_device_descriptor device_descriptor = {
        ...
        .bDeviceClass    = 2,      // Specified in interface descriptor
        .bDeviceSubClass = 0,      // No subclass
        .bDeviceProtocol = 0,      // No protocol
        .bMaxPacketSize0 = 64,     // Max packet size for ep0
        .idVendor        = 0x152a, // Your vendor id
        .idProduct       = 0x80c0, // Your product ID
...
static const struct usb_endpoint_descriptor ep1_in = {
        .bLength          = sizeof(struct usb_endpoint_descriptor),
        .bDescriptorType  = USB_DT_ENDPOINT,
        .bEndpointAddress = EP1_IN_ADDR, // EP number 1, OUT from host (rx to device)
...
static const struct usb_endpoint_descriptor ep2_out = {
        .bLength          = sizeof(struct usb_endpoint_descriptor),
        .bDescriptorType  = USB_DT_ENDPOINT,
        .bEndpointAddress = EP2_OUT_ADDR, // EP number 2, IN from host (tx from device)
        .bmAttributes     = USB_TRANSFER_TYPE_BULK,
        .wMaxPacketSize   = 64,
        .bInterval        = 0
};
...
static const unsigned char *descriptor_strings[] = {
        (unsigned char *) "Bosch Sensor",    // Vendor
        (unsigned char *) "BST Application Board" // Product
};
...
void usb_handle_string_descriptor(volatile struct usb_setup_packet *pkt) {
    ...
    if (i == 0) {
        ...
        len = usb_prepare_string_descriptor((uint8_t *)unique_id);
    } else if (i == 0xEE) {
        // Send Microsoft string descriptor
        len = usb_prepare_string_descriptor((uint8_t *)"MSFT100\xEE");
    } else {
...
void usb_handle_setup_packet(void) {
    ...
    uint8_t req_direction = pkt->bmRequestType & 0x80;
    ...
    if (req_direction == USB_DIR_OUT) {
        ...
        if (req == USB_REQUEST_GET_DESCRIPTOR) {
            ...
            uint8_t wStatus[] = {0x00, 0x00};
            usb_start_transfer(usb_get_endpoint_configuration(EP0_IN_ADDR), wStatus, 2);
        } else if (req == USB_REQUEST_MSFT && pkt->wIndex == 0x0004) {
            usb_start_transfer(usb_get_endpoint_configuration(EP0_IN_ADDR), (uint8_t *)&msft_compatible, pkt->wLength);
        } else {
...
void ep1_in_handler(uint8_t *buf, uint16_t len)
{
    if(tx_done == false && len != 0)
    {
        usb_start_transfer(usb_get_endpoint_configuration(EP1_IN_ADDR), NULL, 0);
        tx_done = true;
    }
}
...
{
    while(tx_done == false) tight_loop_contents();
    tx_done = false;
    memcpy(tx_buffer,data, len);
    usb_start_transfer(usb_get_endpoint_configuration(EP1_IN_ADDR), tx_buffer, 64);
}
